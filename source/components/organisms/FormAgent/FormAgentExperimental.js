...
class FormAgentExperimental extends Component {
  ...
  nextQuestion = async () => {
    ...
    if (!nextQuestionFromQueue) {
      ...
      chat.switchAgent(props => (
        <WatsonAgent {...props} initialMessages={['Kan jag hjälpa dig med någon annat?']} />
      ));
  ...
  updateQueue = (queue, answers, questions, currentQuestion) => {
    ...
    if (currentQuestion) {
      ...
      if ('logics' in currentQuestion) {
        ...
        const questionToAddInQueue = questions.find(
          q => q.position === currentQuestion.position + 1
        );
  ...
  dequeue = queue => {
    ...
    // on empty queue
    if (this.queueIsEmpty(queue)) return undefined;
    return queue.shift();
  };
  ...
  handleUserInput = async message => {
    ...
    if (currentQuestion) {
      ...
      await this.setState(
        { answers: { ...answers, [currentQuestion.id]: message } },
        this.nextQuestion
      );
    }
  };
