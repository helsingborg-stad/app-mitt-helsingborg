import type { Question, SummaryItem } from "../types/FormTypes";
  ...
  items.forEach((item) => {
    ...
    if (["arrayText", "arrayNumber", "arrayDate"].includes(item.type)) {
      if (
        ...
        (answers[item.id] as string[]).forEach((a) => {
          if (a && a !== "") isEmpty = false;
        });
      }
    }
  });
...
const evaluateOperatorInArray = (
  conditionAsArray: EvaluatedValue[],
  operator: Operator,
  index: number
) => {
  if (operator === "!") {
    ...
    if (operator === "&&")
      conditionAsArray[index - 1] = (conditionAsArray[index - 1] &&
        conditionAsArray[index + 1]) as EvaluatedValue;
    if (operator === "||")
      conditionAsArray[index - 1] = (conditionAsArray[index - 1] ||
        conditionAsArray[index + 1]) as EvaluatedValue;
...
export const evaluateConditionalExpression = (
  ...
  const conditionAsArray = condition
    .split(conditionalOperatorSearchPattern)
    .map((string) => string.trim())
    .filter((string) => string !== "");

  const evaluatedAnswers: EvaluatedValue[] = conditionAsArray.map(
    (valueOrOperator) => {
